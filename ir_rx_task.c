...
void example_ir_rx_task(void *arg)
{
    ...
    rmt_config_t rmt_rx_config = RMT_CONFIG_RX(CONFIG_EXAMPLE_RMT_RX_GPIO, example_rx_channel);
    rmt_config(&rmt_rx_config);
    rmt_driver_install(example_rx_channel, 1000, 0);
    ir_parser_config_t ir_parser_config = IR_PARSER_CONFIG((ir_dev_t)example_rx_channel);//??？？
        ...
        if (items) {
            ...
            if (ir_parser->input(ir_parser, items, length) == ESP_OK) 
            {
                if (ir_parser->get_scan_code(ir_parser, &data1,&data2,&data3,&data4) == ESP_OK) 
                {
                    ...
                    {
                        xEventGroupSetBits(APP_event_group,APP_event_ir_rx_flags_BIT);
